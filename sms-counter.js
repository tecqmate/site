...
function getCharCount(text) {
    var ret = (0, getSmsCharCountInfo)(text);
...
function removeNonGsmChars(msgText) {
    ...
    return (0, replaceNonGsmChars)(msgText, replacementChar);
}

  var inp = $('#text');
  inp.on('input', function(e) {
    var text = inp.val();
    var resp = getCharCount(text);
    var ihtml = [];
    ihtml.push(['<strong>Unicode: </strong>', ('GSM' !== resp.encoding ? 'Yes' : 'No')].join(''));
    ihtml.push('<strong>Characters: </strong>' + (resp.charCount));
    ihtml.push('<strong>SMS: </strong>' + (resp.msgCount));
    ihtml.push('<strong>Encoding: </strong>', resp.encoding);
    $('#out').html(ihtml.join(', '));
  });
});